...
export class TextProcessor {
  ...
  private constructor() {
    ...
    // Priority: NVIDIA > Ollama > vLLM (vLLM requires GPU setup)
    if (process.env.NVIDIA_API_KEY) {
      this.selectedLLMProvider = 'nvidia';
    } else if (process.env.OLLAMA_BASE_URL) {
      this.selectedLLMProvider = 'ollama';
    } else if (process.env.VLLM_BASE_URL) {
  ...
  public async processText(text: string): Promise<Array<Triple & { confidence: number, metadata: any }>> {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      try {
        ...
        // Strip markdown code blocks if present (NVIDIA models often wrap JSON in ```json...```)
        const markdownJsonMatch = responseText.match(/```(?:json)?\s*([\s\S]*?)```/);
        if (markdownJsonMatch) {
          responseText = markdownJsonMatch[1].trim();
        }

        // Filter out triples with null objects (model sometimes returns these for statements without objects)
        try {
          const rawTriples = JSON.parse(responseText);
          const validTriples = rawTriples.filter((t: any) => t.object !== null && t.object !== undefined);
          responseText = JSON.stringify(validTriples);
        } catch (e) {
          // If pre-parsing fails, continue with original text
        }
